...
class AppInitializationService {
  ...
  Future<InitializationMetrics> initializeApp() async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('🚀 AppInitializationService: Optimized initialization completed in ${stopwatch.elapsed.inMilliseconds}ms');
        debugPrint('📊 $metrics');
      }
      ...
      if (kIsWeb) {
        ...
        try {
          ...
          if (fe.code == 'duplicate-app') {
            if (kDebugMode) debugPrint('⚠️ Firebase duplicate-app detected - ignoring');
          } else {
            rethrow;
          }
        }
  ...
  Future<void> _initializeAuthentication() async {
    ...
    try {
      ...
      await authStateService.initializeAuthState().timeout(
        ...
        onTimeout: () {
          if (kDebugMode) debugPrint('⏰ Authentication state initialization timed out');
      ...
      if (kDebugMode) {
        debugPrint('✅ Authentication initialized in ${stopwatch.elapsed.inMilliseconds}ms');
      }
    } catch (e) {
      stopwatch.stop();
  ...
  Future<void> _initializeDeepLinking() async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('✅ Deep linking initialized in ${stopwatch.elapsed.inMilliseconds}ms');
      }
    } catch (e) {
      stopwatch.stop();
  ...
  Future<void> _initializeNotifications() async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('✅ Notifications initialized in ${stopwatch.elapsed.inMilliseconds}ms');
      }
    } catch (e) {
      stopwatch.stop();
  ...
  List<String> getPerformanceRecommendations() {
    ...
    if (metrics.authDuration.inMilliseconds > 2000) {
      recommendations.add('🔐 Cache authentication state to reduce initialization time');
    }
    
    recommendations.add('🚀 Consider implementing service worker for background tasks');
