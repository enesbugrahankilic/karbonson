...
class NicknameService {
  ...
  static String getRandomSuggestion() {
    final randomIndex = DateTime.now().millisecondsSinceEpoch % _availableNames.length;
  ...
  static List<String> getMultipleSuggestions({int count = 5}) {
    ...
    while (suggestions.length < count && usedIndices.length < _availableNames.length) {
      final randomIndex = (baseTime + suggestions.length * 1000) % _availableNames.length;
