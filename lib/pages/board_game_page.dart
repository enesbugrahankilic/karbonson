...
class _BoardGamePageState extends State<BoardGamePage>
  ...
  void initState() {
    ...
    _diceRotationAnimation =
        Tween<double>(begin: 0, end: 2 * math.pi * 5).animate(
      CurvedAnimation(
          parent: _diceAnimationController, curve: Curves.easeInOut),
    );
  ...
  Future<void> _startQuiz(NavigatorState navigator,
      ScaffoldMessengerState messenger, dynamic gameLogic) async {
    ...
    if (resultScore != null) {
      final message =
          gameLogic.onQuizFinished(resultScore, gameLogic.currentPlayer!);
  ...
  Future<void> _showEndGameDialog(BuildContext context, dynamic gameLogic) async {
    ...
    if (widget.isMultiplayer) {
      ...
      if (currentPlayer != null && !_scoreSaved) {
        ...
        final saveMessage = await FirestoreService()
            .saveUserScore(currentPlayer.nickname, totalScore);
        _scoreSaved = true;
        ...
        if (saveMessage == 'Skor kaydedildi.') {
          ...
          final playerIndex = leaderboard.indexWhere((entry) =>
              entry['nickname'] == currentPlayer.nickname &&
          ...
          transitionBuilder: (BuildContext dialogContext,
            ...
            return ScaleTransition(
              scale: Tween<double>(begin: 0.5, end: 1.0).animate(
                  CurvedAnimation(
                      parent: animation1, curve: Curves.elasticOut)),
              child: FadeTransition(
                ...
                child: AlertDialog(
                  ...
                  title: Row(
                    children: [
                      const Text('ÇOK OYUNCULU OYUN BİTTİ! ',
                          style: TextStyle(
                              color: Color(0xFF4CAF50), fontSize: 24)),
                      AnimatedScale(
                  ...
                  actions: <Widget>[
                    TextButton(
                      ...
                      child: const Text('Liderlik Tablosunu Gör',
                          style: TextStyle(
                              color: Color(0xFF1E88E5), fontSize: 18)),
                    ),
                  ],
                ),
              ),
      ...
      String saveMessage = '';
      if (!_scoreSaved) {
        saveMessage = await FirestoreService()
            .saveUserScore(gameLogic.player.nickname, totalScore);
        _scoreSaved = true;
      ...
      if (saveMessage == 'Skor kaydedildi.') {
        final leaderboard = await FirestoreService().getLeaderboard();
        final playerIndex = leaderboard.indexWhere((entry) =>
            entry['nickname'] == gameLogic.player.nickname &&
            entry['score'] == totalScore);
        if (playerIndex != -1 && playerIndex < 10) {
          isInTop10 = true;
        }
          ...
          return ScaleTransition(
            ...
            child: FadeTransition(
              ...
              child: AlertDialog(
                ...
                title: Row(
                  children: [
                    Text('${AppLocalizations.gameOver}! ',
                        style: const TextStyle(
                            color: Color(0xFF4CAF50), fontSize: 24)),
                    AnimatedScale(
                      scale: animation1.value,
                ...
                actions: <Widget>[
                  TextButton(
                    ...
                    child: const Text('Liderlik Tablosunu Gör',
                        style:
                            TextStyle(color: Color(0xFF1E88E5), fontSize: 18)),
                  ),
                ],
              ),
            ),
  ...
  Future<void> _initializeMultiplayerGame(
      BuildContext context, MultiplayerGameLogic gameLogic) async {
                  ...
                  child: ConstrainedBox(
                    constraints: BoxConstraints(
                      minHeight: constraints.maxHeight -
                          kToolbarHeight -
                          36, // Account for app bar and padding
                    ),
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        if (!widget.isMultiplayer)
                          Card(
                            ...
                            color: ThemeColors.getPlayerInfoCardBackground(
                                context),
                            child: Padding(
                              padding: const EdgeInsets.symmetric(
                              ...
                              child: ListTile(
                                ...
                                title: Column(
                                  ...
                                  children: [
                                    Text(
                                        '${AppLocalizations.player}: ${gameLogic.player.nickname}',
                                        style: const TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 18)),
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Text(gameLogic.timeElapsedFormatted,
                                            style: TextStyle(
                                                fontSize: 16,
                                                fontWeight: FontWeight.bold,
                                                color:
                                                    ThemeColors.getGameTimeText(
                                                        context))),
                                      ],
                                    ),
                                  ],
                                ...
                                subtitle: Text(
                                    'Quiz Puanı: ${gameLogic.player.quizScore}',
                                    style: const TextStyle(fontSize: 16)),
                                trailing: gameLogic.player.turnsToSkip > 0
                                    ? Text(
                                        'PAS TUR: ${gameLogic.player.turnsToSkip}',
                                        style: const TextStyle(
                                            color: Colors.red,
                                            fontWeight: FontWeight.bold,
                                            fontSize: 16))
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  ...gameLogic.currentRoom.players
                                      .map((player) {
                                    final isCurrentPlayer = player.id ==
                                        gameLogic.currentPlayer?.id;
                                    final isCurrentTurn = gameLogic
                                            .currentRoom
                                            .players[gameLogic
                        ...
                        Card(
                          ...
                          shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(18)),
                          color:
                              ThemeColors.getPlayerInfoCardBackground(context),
                          child: Padding(
                            padding: const EdgeInsets.symmetric(
                                vertical: 18.0, horizontal: 12.0),
                            child:
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final playersHere = widget.isMultiplayer
            ? gameLogic.currentRoom.players
                .where((p) => p.position == index)
                .toList()
            : gameLogic.currentPlayer != null &&
                    gameLogic.currentPlayer.position == index
  ...
  Widget _buildTile(BuildContext context, BoardTile tile,
      List<dynamic> playersHere, bool isMultiplayer) {
    Color tileColor;
    IconData tileIcon;
    switch (tile.type) {
      case TileType.start:
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: tileColor,
        borderRadius: BorderRadius.circular(8),
        border: playersHere.isNotEmpty
            ? Border.all(color: Colors.black, width: 3)
            : null,
        boxShadow: playersHere.isNotEmpty
            ? [
                const BoxShadow(
                ...
                child: Row(
                  ...
                  children: playersHere.map((player) {
                    ...
                    return Icon(Icons.person_pin_circle,
                        color: playerColor, size: 28 / playersHere.length);
                  }).toList(),
                ),
              ),
          ],
  ...
  Widget _buildDiceArea(
      BuildContext context, dynamic gameLogic, BoardTile currentTile) {
    bool isDisabled = gameLogic.isDiceRolling ||
    ...
    if (widget.isMultiplayer) {
      final currentPlayer = gameLogic
          .currentRoom.players[gameLogic.currentRoom.currentPlayerIndex];
      turnIndicator = 'Sıra: ${currentPlayer.nickname}';
      isDisabled =
          isDisabled || !gameLogic.isMyTurn || currentPlayer.turnsToSkip > 0;
      buttonText = gameLogic.isQuizActive
          ? AppLocalizations.quizOpen
          : (!gameLogic.isMyTurn
              ? 'Bekleniyor...'
              : (currentPlayer.turnsToSkip > 0
                  ? '${AppLocalizations.skipTurns} (${currentPlayer.turnsToSkip})'
                  : AppLocalizations.rollDiceEllipsis));
    } else {
      isDisabled =
          isDisabled || (gameLogic.currentPlayer?.turnsToSkip ?? 0) > 0;
      buttonText = gameLogic.isQuizActive
          ? AppLocalizations.quizOpen
          : ((gameLogic.currentPlayer?.turnsToSkip ?? 0) > 0
              ? '${AppLocalizations.skipTurns} (${gameLogic.currentPlayer?.turnsToSkip ?? 0})'
              : AppLocalizations.rollDiceEllipsis);
    }
      ...
      children: [
        if (widget.isMultiplayer && turnIndicator.isNotEmpty)
          Container(
            ...
            decoration: BoxDecoration(
              color: ThemeColors.getTurnIndicatorBackground(
                  context, gameLogic.isMyTurn),
            ...
            child: Text(
              ...
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
                color: ThemeColors.getTurnIndicatorText(
                    context, gameLogic.isMyTurn),
              ),
              textAlign: TextAlign.center,
            ),
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              return Transform.rotate(
                angle:
                    gameLogic.isDiceRolling ? _diceRotationAnimation.value : 0,
                child: gameLogic.lastDiceRoll > 0
                    ? Text('🎲 ${gameLogic.lastDiceRoll}',
                        style: TextStyle(
                            fontSize: 32,
                            fontWeight: FontWeight.w900,
                            color: ThemeColors.getDiceValueText(context)))
                    : Text('🎲 Zar At!',
                        style: TextStyle(
                            fontSize: 32,
                            fontWeight: FontWeight.w900,
            ...
            ElevatedButton(
              onPressed: isDisabled
                  ...
                  : () async {
                      ...
                      if (rollResult == -1) {
                        _showTopNotification('Tur Atlandı! Cezanız Bitmedi.',
                            color: Colors.orange,
                            duration: const Duration(seconds: 3));
                      } else if (rollResult > 0) {
                        final currentTileAfterMove = gameLogic
                            .board.tiles[gameLogic.currentPlayer!.position];

                        // 1. Kare etkisini uygula
                        final message = widget.isMultiplayer
                            ? gameLogic.applyTileEffect(
                                gameLogic.currentPlayer!, currentTileAfterMove)
