...
void main() {
  group('TwoFactorAuthResult - Core Functionality', () {
    ...
    test('should create 2FA required result with multi-factor resolver', () {
      ...
      final result = TwoFactorAuthResult.requires2FA(
        ...
        multiFactorResolver:
            null, // Will be set by Firebase during real 2FA flow
    ...
    test('should provide Turkish localized messages', () {
      ...
      expect(
          failureResult.getTurkishMessage(),
          anyOf([
            contains('başarısız'),
            contains('hata'),
          ]));
  ...
  group('Phone Number Validation - Turkish Standards', () {
    ...
    test('should reject invalid +90 format numbers', () {
      ...
      expect(Firebase2FAService.isValidPhoneNumber('+90512345678'),
          false); // 12 chars
      expect(Firebase2FAService.isValidPhoneNumber('+9051234567890'),
          false); // 14 chars
    });
    ...
    test('should reject invalid 05 format numbers', () {
      ...
      expect(Firebase2FAService.isValidPhoneNumber('0551234567'),
          false); // 10 chars
      expect(Firebase2FAService.isValidPhoneNumber('055123456789'),
          false); // 12 chars
    });
    ...
    test('should handle formatted phone numbers with separators', () {
      ...
      // Invalid formats
      expect(Firebase2FAService.isValidPhoneNumber('+90-512-345-6789'),
          false); // Too many digits
      expect(Firebase2FAService.isValidPhoneNumber('055-12-345-678'),
  ...
  group('2FA Error Handling and Turkish Localization', () {
    ...
    test('should handle unknown error codes gracefully', () {
      ...
      final errorMessage =
          Firebase2FAService.getTurkishErrorMessage(unknownErrorCode);
  ...
  group('Advanced 2FA Scenarios', () {
    test('should handle multiple failed verification attempts', () {
      ...
      for (final attempt in attempts) {
        final result =
            TwoFactorAuthResult.failure('Verification failed: $attempt');
        expect(result.isSuccess, false);
